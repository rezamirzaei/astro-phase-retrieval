...
import { AlgoInfo, ApiService, CompareResponse, FitsFile } from '../core/api.service';
...
@Component({
  ...
  template: `
    <h2>Compare on Real Observations</h2>
    <p class="text-muted mb-16">
      Run several solvers on the same real FITS observation and inspect which reconstruction
      produces the strongest verification evidence in the result views.
    </p>
    <mat-card class="mb-16">
      <mat-card-content class="flex-row flex-wrap">
        <mat-form-field>
          <mat-label>Real FITS File</mat-label>
        ...
        <button mat-raised-button color="accent" (click)="compare()" [disabled]="loading()">
          {{ loading() ? 'Comparing…' : 'Compare Real Data' }}
        </button>
      </mat-card-content>
    ...
    @if (response()) {
      ...
      <table mat-table [dataSource]="response()!.results" class="mat-elevation-z1 mb-16">
        ...
        <ng-container matColumnDef="artifacts"><th mat-header-cell *matHeaderCellDef>Verification</th><td mat-cell *matCellDef="let j">{{ j.artifacts.includes('reference_validation.json') ? 'Reference baseline' : 'Metrics only' }}</td></ng-container>
        <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th><td mat-cell *matCellDef="let j"><a mat-button [routerLink]="['/results', j.id]">Inspect</a></td></ng-container>
...
export class CompareComponent implements OnInit, OnDestroy {
  ...
  cols = ['algorithm', 'strehl', 'rms', 'artifacts', 'actions'];
  ...
  ngOnInit(): void {
    ...
    this.api.listAlgorithms().subscribe(a => {
      this.algos.set(a);
      this.selectedAlgorithms = a.filter(algo => algo.key !== 'phase_diversity').map(algo => algo.key);
    });
  }
  ...
  compare(): void {
    if (!this.selectedFile) {
      this.snack.open('Select a real FITS observation first', 'OK', { duration: 2000 });
      return;
    }
    ...
    this.api.compare({
      ...
      error: e => {
        this.loading.set(false);
        this.snack.open(e?.error?.detail || 'Compare failed', 'OK', { duration: 4000 });
      },
    });
  }
