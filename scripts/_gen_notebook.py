...
Run:  python scripts/_gen_notebook.py
"""
...
code(
    ...
    [
        ...
        "    AlgorithmConfig, AlgorithmName, BetaSchedule, default_hst_config,",
        ")",
...
code(
    ...
    [
        ...
        "    plot_convergence, plot_encircled_energy, plot_psf_comparison,",
        "    plot_psf_cross_sections, plot_radial_profile,",
        "    plot_reconstructed_psf, plot_recovered_phase,",
        "    plot_wavefront_3d, save_figure,",
        ")",
        "",
...
code(
    "34b",
    [
        "from src.metrics.quality import compute_mtf, compute_phase_structure_function, compute_ssim",
...
code(
    "38",
    [
        "from src.data.downloader import available_presets, download_all_presets, list_cached_fits",
...
code(
    ...
    [
        "from src.visualization.plots import (",
        "    plot_algorithm_comparison, plot_algorithm_dashboard,",
        "    plot_strehl_rms_bar,",
        ")",
        "",
...
md(
    ...
    [
        ...
        "",
        "When `RUN_PINN = False`, the figure/table still show the classical baselines"
        " and report the current PINN status.",
    ],
)
code(
    "48d",
...
# ═══════════════════════════════════════════════════════════════════════
# Cell 50 — Algorithm Dashboard
# ═══════════════════════════════════════════════════════════════════════
code(
    "50",
    [
        "fig = plot_algorithm_dashboard(",
        "    psf_data_resized, comparison_results, support, pupil,",
        ")",
...
# ═══════════════════════════════════════════════════════════════════════
# Cell 52–53 — Convergence comparison
# ═══════════════════════════════════════════════════════════════════════
md(
    "52",
    [
        "## 20 \u2014 Convergence Comparison",
        "",
        "Focal-plane cost vs. iteration for all algorithms on a single log-scale axis.",
    ],
)
code(
    "53",
    [
        ...
        "for _i, (name, res) in enumerate(comparison_results.items()):",
        "    ax.semilogy(",
        "        res.cost_history,",
        '        label=f"{name} (Strehl={res.strehl_ratio:.3f})",',
        "        linewidth=1.5,",
        "    )",
...
# ═══════════════════════════════════════════════════════════════════════
# Cell 54–55 — Results summary table
# ═══════════════════════════════════════════════════════════════════════
md(
    "54",
    [
...
code(
    ...
    [
        ...
        "console = Console()",
        "table = Table(",
        '    title=f"Phase Retrieval Results \\u2014 Real HST Data"',
        '    f" ({len(comparison_results)} Algorithms)",',
        "    show_lines=True,",
        ")",
        'table.add_column("Algorithm", style="bold cyan")',
...
md(
    "55a",
    [
        "## 22b — Real-Data Verification and Reporting",
        "",
        "The notebook now mirrors the real-data verification workflow exposed in the web app."
        " We compare reconstructions against curated STScI-style baselines when available and"
        " then write the same machine-readable verification artifacts used by the main pipeline.",
    ],
)
code(
    "55b",
    [
        ...
        "verification_rows = []",
        "for name, res in comparison_results.items():",
        "    baseline = compare_against_reference(",
        "        observed_psf=psf_data_resized.image,",
        "        reconstructed_psf=res.reconstructed_psf,",
        "        pixel_scale_arcsec=psf_data_resized.pixel_scale_arcsec,",
        "        telescope=psf_data_resized.telescope,",
        "        detector=str(psf_data_resized.metadata.get('detector', '')),",
        "        filter_name=psf_data_resized.filter_name,",
        "    )",
        "    verification_rows.append({",
        "        'algorithm': name,",
        "        'ssim': compute_ssim(psf_data_resized.image, res.reconstructed_psf),",
        "        'fwhm_agreement': baseline.get('summary', {}).get('fwhm_agreement', 'n/a'),",
        "        'encircled_energy_agreement': baseline.get('summary', {}).get('encircled_energy_agreement', 'n/a'),",
        "        'baseline_key': baseline.get('baseline', {}).get('key', 'none'),",
        "    })",
        "",
        "verification_rows = sorted(verification_rows, key=lambda row: row['ssim'], reverse=True)",
        "print(json.dumps(verification_rows, indent=2))",
        "best_name = verification_rows[0]['algorithm']",
        "best_result = comparison_results[best_name]",
        "best_reference = compare_against_reference(",
        "    observed_psf=psf_data_resized.image,",
        "    reconstructed_psf=best_result.reconstructed_psf,",
        "    pixel_scale_arcsec=psf_data_resized.pixel_scale_arcsec,",
        "    telescope=psf_data_resized.telescope,",
        "    detector=str(psf_data_resized.metadata.get('detector', '')),",
        "    filter_name=psf_data_resized.filter_name,",
        ")",
        "",
        "print(f'Best verification candidate: {best_name}')",
        "print(json.dumps(best_reference, indent=2))",
    ],
)
code(
    "55c",
    [
        "from src.reporting import render_evaluation_markdown, write_evaluation_report, build_evaluation_payload",
        "",
        "verification_dir = config.output_dir / 'verification'",
        "verification_dir.mkdir(parents=True, exist_ok=True)",
        "",
        "best_algorithm = next(alg for alg in algorithms_to_compare if alg.value.upper() == best_name)",
        "best_cfg = AlgorithmConfig(name=best_algorithm, max_iterations=500, beta=0.9, random_seed=42)",
        "best_zernike = zernike_decomposition(best_result.recovered_phase, support, n_terms=15)",
        "pupil_summary = {",
        "    'grid_size': pupil.grid_size,",
        "    'support_pixels': int(np.sum(pupil.amplitude > 0)),",
        ...
        "}",
        "provenance = {",
        "    'psf': psf_data_resized.metadata,",
        "    'algorithm': {",
        "        'name': best_cfg.name.value,",
        "        'max_iterations': best_cfg.max_iterations,",
        "        'beta': best_cfg.beta,",
        "        'beta_schedule': best_cfg.beta_schedule.value,",
        "        'momentum': best_cfg.momentum,",
        "        'tv_weight': best_cfg.tv_weight,",
        "        'noise_model': best_cfg.noise_model.value,",
        "        'n_starts': best_cfg.n_starts,",
        "        'random_seed': best_cfg.random_seed,",
        "    },",
        "    'pupil': pupil_summary,",
        "    'reference_validation_available': bool(best_reference),",
        "}",
        "metrics = {",
        "    'algorithm': best_result.algorithm.value,",
        "    'n_iterations': best_result.n_iterations,",
        "    'converged': best_result.converged,",
        "    'strehl_ratio': best_result.strehl_ratio,",
        "    'rms_phase_rad': best_result.rms_phase_rad,",
        "    'ssim': compute_ssim(psf_data_resized.image, best_result.reconstructed_psf),",
        "    'radial_profile_error': compute_radial_profile_error(psf_data_resized.image, best_result.reconstructed_psf),",
        "    'encircled_energy_error': compute_encircled_energy_error(psf_data_resized.image, best_result.reconstructed_psf),",
        "    'elapsed_seconds': best_result.elapsed_seconds,",
        "    'timestamp': best_result.timestamp.isoformat(),",
        "    'reference_validation': best_reference,",
        "}",
        "evaluation_payload = build_evaluation_payload(",
        "    psf_metadata={**psf_data_resized.metadata, 'obs_id': psf_data_resized.obs_id, 'source_kind': 'fits', 'source_filename': fits_paths[0].name},",
        "    algorithm_name=best_cfg.name.value,",
        "    algorithm_config=best_cfg.model_dump(mode='json'),",
        "    pupil_summary=pupil_summary,",
        "    metrics=metrics,",
        "    zernike_coefficients=best_zernike,",
        ")",
        "report_paths = write_evaluation_report(evaluation_payload, verification_dir)",
        "(verification_dir / 'reference_validation.json').write_text(json.dumps(best_reference, indent=2))",
        "(verification_dir / 'provenance.json').write_text(json.dumps(provenance, indent=2))",
        "",
        "print(render_evaluation_markdown(evaluation_payload).split('## External Reference Baseline')[0])",
...
md(
    ...
    [
        "## 22 \u2014 Export Results",
        "",
        "Every result is a Pydantic model \u2014 we serialise the non-array metadata"
...
code(
    ...
    [
        ...
        '    f"all outputs saved to {config.output_dir}"',
        ")",
    ],
)
...
md(
    ...
    [
        ...
        "",
        "- **Real data, not synthetic**: the PSF was recorded by HST\u2019s WFC3/UVIS"
        " detector from actual stellar photons collected in orbit.",
        "- **7 iterative algorithms compared by default**: from the classic ER/GS"
        " (1972\u20131982) through RAAR (2005) to state-of-the-art Wirtinger Flow"
        " (2015), Douglas-Rachford, and ADMM (2018). The optional `pinn` solver"
        " can also be run when PyTorch is installed and `RUN_PINN = True`.",
        "- **State-of-the-art enhancements**: Nesterov momentum acceleration,"
        " adaptive \u03b2 cosine annealing, TV regularization, Poisson noise model,"
        " and multi-start optimization are built into the base class.",
        "- **Comprehensive metrics**: Strehl ratio, RMS wavefront error, Zernike"
        " decomposition, MTF, SSIM, and Phase Structure Function.",
        "- **Phase retrieval works**: all algorithms successfully recover a"
        " wavefront that, when forward-modelled, reproduces the observed PSF.",
        "- **Algorithm trade-offs**: HIO/RAAR/DR converge faster than ER/GS."
        " Wirtinger Flow with spectral init provides a strong starting point."
        " ADMM handles regularization naturally.",
        "- **Zernike decomposition** reveals the dominant aberrations \u2014 directly"
        " actionable for telescope alignment and optical design.",
        "- **Pydantic validation** catches misconfiguration early \u2014 wrong image"
        " shapes, out-of-range parameters, and type errors are all caught at"
        " model construction.",
        "- **Modular design**: swapping telescopes (HST \u2192 JWST), filters, or"
        " algorithms requires changing only the configuration \u2014 not the code.",
        "",
        "This is the same fundamental technique used operationally by NASA/STScI"
        " for HST focus monitoring and JWST mirror alignment \u2014 scaled to a clean,"
        " reproducible, state-of-the-art implementation.",
    ],
)
